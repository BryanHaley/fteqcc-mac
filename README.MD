...
This project builds fteqcc for macOS (both Intel macOS 10.9+ and Apple Silicon macOS 11.0+). Forked from the source tarball available at [fteqcc.org](https://www.fteqcc.org/) circa revision 6202.
...
Assumes you have brew installed and thus ```brew --prefix``` will return a valid path. If you don't have brew installed, just copy the ```fteqcc``` binary to a location of your choosing. For example:
```
cp fteqcc /Users/bryan/Projects/MyCoolQuakeProject/
```

## Usage

Refer to the [fteqcc manual](https://www.fteqcc.org/dl/fteqcc_manual.txt).

## Original readme.txt

Note: this readme is for QCLib, the QuakeC runtime. This project builds qcc, the compiler for QuakeC.
